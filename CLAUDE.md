...
- The contact form uses Netlify Forms (`data-netlify="true"`, with a honeypot field named `company`). It must stay in static HTML so Netlify can detect it at deploy.
...
- The cover's background is flattened to exactly `--paper`, so the artwork blends into the page. Don't add borders or shadows to it. If you replace the cover, flatten its background the same way.
...
- Confirm the Instagram handle (the placeholder is `instagram.com/adriaivanmusic`)
- About text, and contact emails for booking and press
